...
const botSay = (data) => {
  return[
    "halo, perkenalkan nama saya 404_notfoundbot. Siapa nama kamu ?",
...
function finishing() {
  ...
  container[0].style.filter = "blur(8px)"
  setTimeout(() =>{
  pertanyaan.innerHTML = `Thank U ya ${usersData[0]} udah mampir ke 404_notfoundbot, kali kali ajakin aku ${usersData[2]} bareng ya. 😉 `
 loaders.style.display = "none"
 container[0].style.filter = "none"
  }, [1500])
...
function botEnd() {
  ...
  alert
(`mampir ke Instagram ku yuk, clik Weeeeeeee_0 di atas, follow ya. ${usersData[0]}`)
